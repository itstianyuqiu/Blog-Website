...
<div id="signup_frame_signup">
    ...
    <div id="signup_control">
        ...
        <button type="button" id="btn_signup" value="Sign up">Sign up</button>
    </div>
...
<script>
    $("#btn_signup").click(function () {
        ...
        $.post("/UserNewAccountServlet", {
            ...
            'dob_day':  $('#form_dob_day').val(),
            'dob_month': $('#form_dob_month').val(),
            'dob_year':$('#form_dob_year').val(),
            'gender': $('.gender').val(),
        }, function (xhr) {
            if (xhr == "success") {
    ...
    $('#signup_username').on('blur', function () {// when entry value changes
        ...
        $.post(//post data to the URL
            ...
            , function (data) {//callback function when it's successful
                ...
                if (errMsg == 1) {
                    $('#display b').text("No  user name input!");
                    $('#display b').css('color','red');
                } else if (errMsg == 2) {
                    $('#display b').text("Veiled username");
                    $('#display b').css('color','red');
                } else if (errMsg == 3) {
